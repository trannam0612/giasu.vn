...
import 'package:giasu_vn/common/shared/data/models/result_delete_parent_saved.dart';
...
class ListParentSavedController extends GetxController {
  ...
  ResultDeleteParentSaved resultDeleteParentSaved = ResultDeleteParentSaved();
  ...
  Future<void> deleteParentSaved(int idPH) async {
    print('parentSaved');
    // await Future.delayed(Duration(milliseconds: 1));
    // Get.dialog(DialogLoading());
    String token = SpUtil.getString(ConstString.token);
    ResultData res = await homeRepositories.deleteParentSaved(token, idPH);
    resultDeleteParentSaved = resultDeleteParentSavedFromJson(res.data);
    if (resultDeleteParentSaved.data != null) {
      // Get.back();
      Utils.showToast(resultDeleteParentSaved.data.message);
    } else {
      Utils.showToast(resultParentSaved.error.message);
    }
    update();
